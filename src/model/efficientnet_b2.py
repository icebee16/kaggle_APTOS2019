...
def get_efficientnet_b2(task, weight=None, pretrained=False):
    """
    Parameters
    ----------
    task: str
    ...
    if pretrained:
        model.load_state_dict(torch.load(Path(__file__).parents[2] / "model" / "pretrain" / "efficientnet-b2.pth"))
...
if __name__ == "__main__":
    net = get_efficientnet_b2("classifier", pretrained=True)
    x = torch.randn(16, 3, 512, 512)
    y = net(x)
    assert torch.Size([16, 5]) == y.size()

    net = get_efficientnet_b2("regression", pretrained=True)
    x = torch.randn(16, 3, 512, 512)
    y = net(x)
