...
class executer(object):
    ...
    def data_preprocess(self):
        ...
        self.predict = np.zeros((len(test_dataset))).reshape((-1, 1))  # only test process
    ...
    def __load_dataset(self, dataset_name, dataset_params):
        ...
        """
        dataset = CentralTestDataset(**dataset_params)
    ...
    def load_model(self):
        ...
        with open(model_dir + "/{}_{}.txt".format(get_version(), self.fold), mode="r") as f:
            coef = [float(c.strip()) for c in f.readlines()]
        self.estimator.set_coef(coef)
    ...
    def inference(self):
        ...
        for i, data in enumerate(self.test_loader):
            ...
            if torch.cuda.is_available():
                outputs = outputs.cpu()
            self.predict[i * batch_size:(i + 1) * batch_size] = outputs.detach().numpy()
