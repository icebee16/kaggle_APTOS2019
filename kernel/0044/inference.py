...
class executer(object):
    ...
    def __load_config(self):
        config_dict = {'summary': {'task': 'classifier', 'fold': 1}, 'dataloader': {'dataset': 'bencolor', 'batch_size': 32, 'preprocess': None, 'transform': {'train': [{'function': 'RandomRotation', 'params': {'degrees': 180}}, {'function': 'RandomHorizontalFlip'}, {'function': 'Resize', 'params': {'size': 256}}, {'function': 'ToTensor'}, {'function': 'Normalize', 'params': {'mean': [0.485, 0.456, 0.406], 'std': [0.229, 0.224, 0.225]}}], 'valid': [{'function': 'RandomRotation', 'params': {'degrees': 180}}, {'function': 'RandomHorizontalFlip'}, {'function': 'Resize', 'params': {'size': 256}}, {'function': 'ToTensor'}, {'function': 'Normalize', 'params': {'mean': [0.485, 0.456, 0.406], 'std': [0.229, 0.224, 0.225]}}], 'test': [{'function': 'RandomRotation', 'params': {'degrees': 180}}, {'function': 'RandomHorizontalFlip'}, {'function': 'Resize', 'params': {'size': 256}}, {'function': 'ToTensor'}, {'function': 'Normalize', 'params': {'mean': [0.485, 0.456, 0.406], 'std': [0.229, 0.224, 0.225]}}]}}, 'train': {'condition': {'epoch': 150, 'early_stopping_rounds': 10, 'verbose': 1, 'seed': 1116}, 'model': {'name': 'efficientnet_b0', 'pretrained': True}, 'optimizer': {'algorithm': 'Adam', 'params': {'lr': 0.001, 'weight_decay': 1e-05}}, 'scheduler': {'algorithm': 'StepLR', 'params': {'step_size': 5, 'gamma': 0.1}}, 'criterion': {'algorithm': 'CrossEntropyLoss'}}, 'inference': None}
        return config_dict
    ...
    def __load_dataset(self, dataset_name, dataset_params):
        ...
        """
        dataset = BencolorTestDataset(**dataset_params)
